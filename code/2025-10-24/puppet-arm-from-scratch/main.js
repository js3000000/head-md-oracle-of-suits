...
// ----------------------
// MediaPipe Hands input
// ----------------------
// This replaces/augments mouse input by using the index-finger tip.
// It starts/stops dragging when index+thumb are pinched.
// Loads mediapipe scripts dynamically so you don't need to edit HTML.
...
(function initHandTracking() {
    ...
    // hidden video feed (used by MediaPipe Camera and as the scene background)
    ...
    video.muted = true;
    video.autoplay = true;
    // keep preview mirrored for user, but video texture flip handled separately
    video.style.transform = 'scaleX(-1)';
    // optional: append preview small window
    // document.body.appendChild(video);

    // small visual cursor at fingertip (optional)
    const fingerCursor = new THREE.Mesh(
    ...
    // will hold the Three VideoTexture
    let videoTexture = null;

    // load mediapipe scripts and start camera
    Promise.all([
        loadScript(MP_BASE + 'hands/hands.js'),
        loadScript(MP_BASE + 'camera_utils/camera_utils.js')
    ]).then(() => {
        const hands = new Hands({ locateFile: (f) => MP_BASE + 'hands/' + f });
        hands.setOptions({
            maxNumHands: 1,
            modelComplexity: 1,
            minDetectionConfidence: 0.6,
            minTrackingConfidence: 0.5,
            selfieMode: true // let MediaPipe treat frames as selfie
        });
        ...
        hands.onResults((results) => {
            ...
            // pointer mapping (mirror compensated)
            pointer.x = - (indexTip.x * 2 - 1);
            pointer.y = - (indexTip.y * 2 - 1);

            // show cursor in world space
            ...
            if (worldPos) {
                fingerCursor.position.copy(worldPos);
            ...
            // near-grab check (screen-space)
            const sphereWorldPos = new THREE.Vector3();
            sphere.getWorldPosition(sphereWorldPos);
            const sphereScreen = worldToScreenPixels(sphereWorldPos);
            const sx = sphereScreen.x, sy = sphereScreen.y;
            const dpx = sx - screenX, dpy = sy - screenY;
            const distPx = Math.sqrt(dpx * dpx + dpy * dpy);
            const allowNearGrab = distPx <= grabRadiusPx;

            if ((isPinching || allowNearGrab) && !isDragging) {
                if (!audioCtx) initAudio();
                if (audioCtx && audioCtx.state === 'suspended') audioCtx.resume();
                ...
                updateDrag(pointer);
                lastPinchState = true;
            } else if ((isPinching || allowNearGrab) && isDragging) {
                updateDrag(pointer);
                lastPinchState = true;
            } else if (!isPinching && isDragging && lastPinchState) {
                endDrag();
                lastPinchState = false;
            }
        });

        // start camera (uses MediaPipe Camera util)
        ...
        cam.start().then(() => {
            // create VideoTexture from the same video element and use it as scene background
            videoTexture = new THREE.VideoTexture(video);
            videoTexture.minFilter = THREE.LinearFilter;
            videoTexture.magFilter = THREE.LinearFilter;
            videoTexture.format = THREE.RGBFormat;
            // mirror horizontally so background behaves like a selfie preview
